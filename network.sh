...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Bank Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-bank.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
  ...
  infoln "Generating CCP files for Bank and Platform"
...
# call the script to create the channel, join the peers of bank and platform,
...
function networkDown() {
  ...
  # stop cim containers also in addition to bank and platform, in case we were running sample to add cim
  COMPOSE_ORG3_BASE_FILES="-f addCim/compose/${COMPOSE_FILE_ORG3_BASE} -f addCim/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_BASE}"
  COMPOSE_ORG3_COUCH_FILES="-f addCim/compose/${COMPOSE_FILE_ORG3_COUCH} -f addCim/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_COUCH}"
  COMPOSE_ORG3_CA_FILES="-f addCim/compose/${COMPOSE_FILE_ORG3_CA} -f addCim/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_CA}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.creditrisk.com docker_peer0.bank.creditrisk.com docker_peer0.platform.creditrisk.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/bank/msp organizations/fabric-ca/bank/tls-cert.pem organizations/fabric-ca/bank/ca-cert.pem organizations/fabric-ca/bank/IssuerPublicKey organizations/fabric-ca/bank/IssuerRevocationPublicKey organizations/fabric-ca/bank/fabric-ca-server.db'
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addCim/fabric-ca/cim/msp addCim/fabric-ca/cim/tls-cert.pem addCim/fabric-ca/cim/ca-cert.pem addCim/fabric-ca/cim/IssuerPublicKey addCim/fabric-ca/cim/IssuerRevocationPublicKey addCim/fabric-ca/cim/fabric-ca-server.db'
...
# use this as the default docker-compose yaml definition for cim
COMPOSE_FILE_ORG3_BASE=compose-cim.yaml
# use this as the docker compose couch file for cim
COMPOSE_FILE_ORG3_COUCH=compose-couch-cim.yaml
# certificate authorities compose file
COMPOSE_FILE_ORG3_CA=compose-ca-cim.yaml
#
