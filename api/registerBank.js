...
async function main() {
    try {
        ...
        if (userIdentity) {
            console.log('An identity for the user "bankUser" already exists in the wallet');
            return;
        }

        // Check to see if we've already enrolled the admin user.
        const adminIdentity = await wallet.get('adminBank');
        if (!adminIdentity) {
        ...
        await wallet.put('bankUser', x509Identity);
        console.log('Successfully registered and enrolled admin user "bankUser" and imported it into the wallet');

    } catch (error) {
        console.error(`Failed to register user "bankUser": ${error}`);
        process.exit(1);
    }
}
